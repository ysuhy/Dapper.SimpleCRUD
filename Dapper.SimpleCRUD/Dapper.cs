...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        /// <summary>
        /// Inserts a row into the database, the row don't have identity id
        /// </summary>
        /// <param name="connection"></param>
        /// <param name="entityToInsert"></param>
        /// <param name="transaction"></param>
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static bool InsertWithoutIdentity(this IDbConnection connection, object entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var name = GetTableName(entityToInsert);
            var sb = new StringBuilder();
            sb.AppendFormat("insert into {0}", name);
            sb.Append(" (");
            BuildInsertParameters(entityToInsert, sb, true);
            sb.Append(") ");
            sb.Append("values");
            sb.Append(" (");
            BuildInsertValues(entityToInsert, sb, true);
            sb.Append(")");
            var r = connection.Execute(sb.ToString(), entityToInsert, transaction, commandTimeout, CommandType.Text);
            return r > 0;
        }
        ...
        private static void BuildInsertValues(object entityToInsert, StringBuilder sb, bool withoutIdentity = false)
        {
            var props = GetScaffoldableProperties(entityToInsert).ToArray();
            for (var i = 0; i < props.Count(); i++)
            {
                var property = props.ElementAt(i);
                if (withoutIdentity == false)// add by ocean
                {
        ...
        private static void BuildInsertParameters(object entityToInsert, StringBuilder sb, bool withoutIdentity = false)
        {
            var props = GetScaffoldableProperties(entityToInsert).ToArray();
        ...
        public class TableNameResolver : ITableNameResolver
        {
            public virtual string ResolveTableName(Type type)
            {
                ...
                if (tableattr != null)
                {
                    string _name = tableattr.Name.ToString();
                    tableName = Encapsulate(_name);
                    try
                    {
